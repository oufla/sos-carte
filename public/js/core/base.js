...
var Finapp = {
  ...
  Dark_Mode: {
    "default": false,
    // Set dark mode as main theme
    local_mode: {
      // Activate dark mode between certain times of the day
      enable: false,
      // Enable or disable local dark mode
      start_time: 20,
      // Start at 20:00
  ...
  Test: {
    ...
    word: "testmode",
    // The word that needs to be typed to activate test mode
    alert: true,
    // Enable or disable alert when test mode is activated
...
if (Finapp.PWA.enable) {
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('__service-worker.js').then(function (reg) {
      return console.log('service worker registered');
    })["catch"](function (err) {
      return console.log('service worker not registered - there is an error.', err);
    });
  }
}
//-----------------------------------------------------------------------
...
document.addEventListener('DOMContentLoaded', function () {
  // Full Carousel
  document.querySelectorAll('.carousel-full').forEach(function (carousel) {
  ...
  // Single Carousel
  document.querySelectorAll('.carousel-single').forEach(function (carousel) {
    return new Splide(carousel, {
      perPage: 3,
      rewind: true,
      type: "loop",
  ...
  // Multiple Carousel
  document.querySelectorAll('.carousel-multiple').forEach(function (carousel) {
    return new Splide(carousel, {
      perPage: 4,
      rewind: true,
      type: "loop",
  ...
  // Small Carousel
  document.querySelectorAll('.carousel-small').forEach(function (carousel) {
    return new Splide(carousel, {
      perPage: 9,
      rewind: false,
      type: "loop",
  ...
  // Slider Carousel
  document.querySelectorAll('.carousel-slider').forEach(function (carousel) {
    return new Splide(carousel, {
      perPage: 1,
      rewind: false,
      type: "loop",
  ...
  // Stories Carousel
  document.querySelectorAll('.story-block').forEach(function (carousel) {
    return new Splide(carousel, {
      perPage: 16,
      rewind: false,
      type: "slide",
...
if (Finapp.Dark_Mode.auto_detect.enable) if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
  pageBody.classList.add("dark-mode");
}
...
if (document.querySelector(".offcanvas") === null) {
  ...
  var CookiesBox = function CookiesBox(time) {
  ...
  window.addEventListener('keypress', function (e) {
    ...
    if (value == word || value === word) {
      ...
      if (Finapp.Test.alert) {
        ...
        var alert = "<div id='alert-toast' class='toast-box toast-center tap-to-close'>" + "<div class='in'>" + "<div class='text'><h1 class='text-light mb-05'>🤖</h1><strong>" + Finapp.Test.alertMessage + "</strong></div></div></div>";
        var wrapper = document.querySelector(".test-alert-wrapper");
        wrapper.innerHTML = alert;
        toastbox('alert-toast');
        setTimeout(function () {
          _this.document.getElementById("alert-toast").classList.remove("show");
        }, 4000);
      }
      testMode();
    }
